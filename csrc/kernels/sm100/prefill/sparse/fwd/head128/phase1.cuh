/*
Sparse Attention Forward Pass (Phase 1) — SM100, h_q=128

Forward attention kernel specialized for 128 query heads. Uses UTCMMA instructions
and Tensor Memory (TMEM). Two-CTA cluster design: B_H=128 split across 2 CTAs,
B_TOPK=128 split across 2 CTAs. The pipeline interleaves KV copy (TMA), Q*K^T MMA,
and softmax scale/exp operations.

Template parameters:
  D_QK         — Head dimension for QK (512 or 576)

Grid: [2*s_q, 1, 1], Cluster: [2, 1, 1]
...
KernelTemplate<D_QK>::sparse_attn_fwd_kernel_devfunc(const SparseAttnFwdParams &params, const TmaParams &tma_params) {
    ...
    ku::barrier_cluster_arrive_relaxed();   // We must add a cluster_sync() here, or TMA from CTA1 may launch before barrier initialization in CTA0
    ...
    if (warpgroup_idx == 0) {
        ...
        for (int k = 0; k < num_k_blocks; ++k) {
            ...
            float cur_pi_max = get_max<B_TOPK/2>(p_float);
...
void run_sparse_fwd_phase1_kernel(const SparseAttnFwdParams& params) {
    ...
    auto tma_Q = cute::make_tma_copy(
        ...
        (typename Kernel::SmemLayoutQTiles<D_QK/64>){}
    );
