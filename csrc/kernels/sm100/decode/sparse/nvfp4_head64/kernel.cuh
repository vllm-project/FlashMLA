    ...
    if (warpgroup_idx == 0) {
        ...
        if (warp_idx == 4 && elect_one_sync()) {
            ...
            static constexpr int tma_coords_step_per_token =
                ...
                MODEL_TYPE == ModelType::V4 ? 576/TMA_K_STRIDE :
                    ...
                    plan.bar_raw_ready[rs.buf_idx].wait(rs.bar_phase);
                    {
                        ...
                        for (int local_row_idx = 0; local_row_idx < ROWS_PER_GROUP; ++local_row_idx) {
                            ...
                            for (int local_col_idx = 0; local_col_idx < COLS_PER_GROUP; ++local_col_idx) {
                                ...
                                ku::nvbf16 sf = (local_col_idx & 1) ? sf_pairs[local_col_idx/2].y : sf_pairs[local_col_idx/2].x;
                                ku::nvbf16x2 out[4];
                                fp4x8_to_bf16x2x4(raw, out);
                                const ku::nvbf16x2 sf2 = {sf, sf};
                                CUTE_UNROLL
                                for (int i = 0; i < 4; ++i)
                                    out[i] = __hmul2(out[i], sf2);
